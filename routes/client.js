...
router.get("/", async (req, res) => {
  ...
  res.render("client/index.ejs", {
    ...
    metaTags: true,
    productInformation: {
      searchKeyWord: ["kanishk", "kumar", "ranjan"],
      shortDescription:
        "This is a deal to be to save your alot money by comparing all the website at once the by on this",
    },
  });
});
...
router.get("/shop/:name", async (req, res) => {
  ...
  if (recommend.length < 8)
    await recommended(
      ...
      recommend.length + 1,
      8 - recommend.length
    ).then((result) => result.forEach((product) => recommend.push(product)));
  res.render("client/product.ejs", {
...
router.get("/shop", async (req, res) => {
  ...
  res.render("client/shop.ejs", {
    products: result,
    search: req.query.search && req.query.search != "" ? req.query.search : "",
  });
});
